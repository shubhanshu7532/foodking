 ...
 * @param {string} phone - The phone number of the restaurant.
 * @param {string} otp - The OTP (One-Time Password) entered by the restaurant.
...
*/
export async function loginWithOTP(phone, otp, first_name, last_name, state, country, city, location, restaurant_name, restaurant_address, category, longitude, latitude) {
    try {
        ...
        if (!user) {
            ...
            if (!restaurant_name) throw new Error('restaurant name is required');
            //  if (!restaurant_name.length < 2 || !restaurant_name.length > 50) throw new Error('Restaurant name must be between 2 and 50 characters');
            if (!restaurant_address) throw new Error('Restaurant address is required');
            if (!category) throw new Error('Category is required');
            // if (!restaurant_address.length < 2 || !restaurant_address.length > 50) throw new Error('Restaurant address must be between 2 and 50 characters');
            ...
            user = await Restaurant.create({ phone, username, first_name, last_name, state, country, city, location, restaurant_name, restaurant_address, longitude, latitude, category, coordinates: { type: 'Point', coordinates: [longitude, latitude] }, }); // create new user
        }
